...
def fetch_papers(query):
    ...
    with st.spinner("🔄 Searching arXiv database and processing papers..."):
        try:
            results = fetch_arxiv_pdfs(query, max_results=3)
            
            if not results:
                st.error("❌ No papers found for this topic. Try different keywords.")
                return False
...
def setup_qa_system():
    """Initialize the QA system with loaded documents"""
    if not st.session_state.docs:
        st.error("❌ No documents loaded. Please load papers first.")
        return
    ...
    try:
        ...
        prompt_template = """You are a helpful AI research assistant. Answer questions based on the provided research papers context.
...
- If information isn't available in the context, say so
- Keep responses informative but concise (under 200 words)
- After answering, ask if the user has any follow-up questions about the research papers
...
def render_conversation_interface():
    ...
    with col2:
        st.markdown("<br><br>", unsafe_allow_html=True)  # Add spacing to align with textarea
...
def process_question(question):
    ...
    if st.session_state.qa_chain is None:
        st.warning("🔄 Setting up QA system...")
        setup_qa_system()
        
        if st.session_state.qa_chain is None:
            st.error("❌ Failed to initialize QA system. Please try reloading the papers.")
            return
    ...
    with st.spinner("🤔 Analyzing papers and generating response..."):
        try:
            ...
            if sources:
                ...
                unique_sources = list(set([src.metadata.get('source', 'Unknown') for src in sources]))
